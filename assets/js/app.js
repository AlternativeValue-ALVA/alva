var App = function () {
    ...
    var handleDashboardCharts = function () {
        ...
        function showTooltip(title, x, y, contents) {
            $('<div id="tooltip" class="chart-tooltip"><div class="date">' + title + '<\/div><div class="label label-success">AI: ' + x / 10 + '%<\/div><div class="label label-important">MEM: ' + x * 40 + '<\/div><\/div>').css({
                position: 'absolute',
                display: 'none',
                top: y - 100,
                width: 75,
        ...
        $("#site_statistics").bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                if (previousPoint != item.dataIndex) {
                    ...
                    showTooltip(Date(), item.pageX, item.pageY, item.series.label + " of " + x + " = " + y);
                }
            } else {
                $("#tooltip").remove();
                previousPoint = null;
        ...
        $("#site_activities").bind("plothover", function (event, pos, item) {
            ...
            if (item) {
                if (previousPoint2 != item.dataIndex) {
                    ...
                    var x = item.datapoint[0].toFixed(2),
                        y = item.datapoint[1].toFixed(2);
                    showTooltip(today.getFullYear(), item.pageX, item.pageY, x);
                }
            }
        });
