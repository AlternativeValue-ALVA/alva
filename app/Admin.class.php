...
var $permissions = array(
	...
	function add_user() {
		...
		$email = (isset($_GET['email']) && strlen($_GET['email']) > 6 && strlen($_GET['email']) < 256 && preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', $_GET['email'])) ? $this->qls->Security->make_safe($_GET['email']) : false;
		...
		$group_id = (isset($_GET['group_id']) && is_numeric($_GET['group_id']) && $_GET['group_id'] > -1) ? $this->qls->Security->make_safe($_GET['group_id']) : false;
			...
			if ($this->qls->User->check_username_existance($username)) {
			...
			$this->add_user_error = REGISTER_SECURITY_ERROR;
			return false;
			}
	...
	$new_email = (isset($_GET['new_email']) && preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', $_GET['new_email'])) ? $this->qls->Security->make_safe($_GET['new_email']) : false;
	...
	$new_group_id = (isset($_GET['new_group_id']) && is_numeric($_GET['new_group_id']) && $_GET['new_group_id'] > -1) ? $this->qls->Security->make_safe($_GET['new_group_id']) : false;
		...
		if ($this->qls->User->check_username_existance($new_username) && $new_username != $row['username']) {
	...
	function add_page($upload) {
		if ($this->qls->user_info['auth_admin_add_page'] == 1) {
			...
			$file_name = $this->qls->Security->make_safe(trim($_FILES['upload']['name']));
				...
				@unlink($this->qls->main_directory . '/' . $page_name);
				return false;
				}
			}
			else {
	...
	function edit_mask() {
		...
		$count = count($this->permissions);
			...
			for ($x = 0; $x < $count; $x++) {
			$this->permissions[$x] = $prefix . $this->permissions[$x];
				if ($this->check_mask_var($_GET[$list[$x]])) {
				$values[] = $_GET[$this->permissions[$x]];
				continue;
				}
				else {
	...
	function send_invite() {
		...
		$to = (isset($_GET['to']) && strlen($_GET['to']) > 6 && preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', $_GET['to'])) ? $_GET['to'] : false;
		$reply_to = (isset($_GET['reply_to']) && strlen($_GET['reply_to']) > 6 && preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', $_GET['reply_to'])) ? $_GET['reply_to'] : false;
