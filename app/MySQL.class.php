...
var $qls;
	...
	$this->real_server = ($this->port !== false) ? $this->server_name . ':' . $this->port : $this->server_name;

	// If the connection is persistent, connect with a persistent connection :P
	$this->connection = ($this->persistent) ? @mysql_pconnect($this->real_server, $username, $password) : @mysql_connect($this->real_server, $username, $password);
		if (!$this->connection) {
		die(mysql_error());
		}
		else {
		$this->database = mysql_select_db($this->name, $this->connection);
			if (!$this->database) {
			die(mysql_error());	
			}
		}
	 ...
	 * Only difference is it will clear the last query
	 */
	...
	/**
	 * http://www.php.net/mysql_get_client_info
	 */
	...
	$this->last_query[] = $query;
	$this->update_queries();
	mysql_query($query, $this->connection) or die(mysql_errno() . ': ' . mysql_error());
	}
